            ...
            @else 
                    ...
                    <div class="cart-box">
                        ...
                        <div class="cart-pro-details">
                            ...
                            <div class="cart-pro-edit">
                                ...
                                <p class="cart-pricing">{{$taxval->currency}}{{number_format($cart->qty * $cart->price, 3)}}</p>
                            </div>
                            ...
                            @if ($cart->addons_id != "")
                                ...
                                @foreach ($addons_id as $key =>  $addons)
                                    @if ($addons_price[$key] == 0)
                                    <div class="cart-addons">
                                        <b>{{$addons_name[$key]}}</b> : <b style="color: #000; text-align: center;">{{$taxval->currency}}0</b>
                                    </div>
                                    @else
                                    <div class="cart-addons">
                                        <b>{{$addons_name[$key]}}</b> : <b style="color: #000; text-align: center;">{{$taxval->currency}}{{number_format($addons_price[$key], 3)}}</b>
                                    </div>
                                    @endif
                                @endforeach
                                </div>
                ...
                <div class="col-lg-4">
                    ...
                    <div class="cart-summary">
                        ...
                        <p class="pro-total">{{ trans('labels.order_total') }} <span>{{$taxval->currency}}{{number_format($order_total, 3)}}</span></p>
                        <p class="pro-total">{{ trans('labels.tax') }} <span>{{$taxval->currency}}{{number_format($tax, 3)}}</span></p>
                        <p class="pro-total" id="delivery_charge_hide">{{ trans('labels.delivery_charge') }}<span id="shipping_charge">{{$taxval->currency}}0</span></p>

                        @if (Session::has('offer_amount'))
                            <p class="pro-total offer_amount">{{ trans('labels.discount') }} ({{Session::get('offer_code')}})</span>
                                <span id="offer_amount">
                                    -{{$taxval->currency}}{{number_format($order_total*Session::get('offer_amount')/100, 3)}}
                                </span>
                            </p>
                        @else
                        ...
                        @if (Session::has('offer_amount'))
                            ...
                            <p class="cart-total">{{ trans('labels.total_amount') }} 
                                <span id="total_amount">
                                    {{$taxval->currency}}{{number_format($order_total+$taxval->delivery_charge+$tax-$order_total*Session::get('offer_amount')/100, 3)}}
                                </span>
                            </p>
                        @else
                            <p class="cart-total">{{ trans('labels.total_amount') }} <span id="total_amount">{{$taxval->currency}}{{number_format($total, 3)}}</span></p>
                        @endif
