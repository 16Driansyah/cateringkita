        ...
        <div class="row">
                    ...
                    <div class="order-details-name">
                        <a href="javascript:void(0)">
                            <a href="{{URL::to('product-details/'.$orders->id)}}">
                                <h3>{{$orders->item_name}} - ({{$orders->variation}} - {{$getdata->currency}}{{$orders->variation_price}})<span>
                                    {{$getdata->currency}}{{number_format($orders->qty * $orders->total_price, 3)}}</span></h3>
                            </a>
                        </a>
                        ...
                        @foreach ($addons_id as $key =>  $addons)                                
                            <div class="cart-addons-wrap">
                                ...
                                <b>{{$addons_name[$key]}}</b> : {{$getdata->currency}}{{number_format($addons_price[$key], 3)}}
                                </div>
                            </div>
                        @endforeach
                        @endif
            ...
            <div class="col-lg-4">
                <div class="order-payment-summary">
                    ...
                    <p>{{ trans('labels.tax') }}<span>{{$getdata->currency}}{{number_format($summery['tax_amount'], 3)}}</span></p>

                    <p>{{ trans('labels.delivery_charge') }} <span>{{$getdata->currency}}{{($summery['delivery_charge'] == 0 ? 0 : number_format(@$summery['delivery_charge'], 3))}}</span></p>

                    @if ($summery['promocode'] !="")
                    <p>{{ trans('labels.discount') }} ({{$summery['promocode']}}) <span>- {{$getdata->currency}}{{number_format($summery['discount_amount'], 3)}}</span></p>
                    @endif
                    <?php
                    ...
                    ?>
                    <p class="order-details-total">{{ trans('labels.total_amount') }} <span>{{$getdata->currency}}{{number_format($total, 3)}}</span></p>
                </div>
