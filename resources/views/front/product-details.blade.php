                ...
                <div class="extra-food-wrap">
                    ...
                    @if (count($paidaddons['value']) != 0)
                        <ul class="list-unstyled extra-food">
                            ...
                            <li>
                                <input type="checkbox" name="addons[]" class="Checkbox" value="{{$addons->id}}" price="{{$addons->price}}" addons_name="{{$addons->name}}">
                                <p>{{$addons->name}} : {{$getdata->currency}}{{number_format($addons->price, 3)}}</p>
                            </li>
                            @endforeach
                            </div>
                        </ul>
                    ...
                    <div class="pro-details-add-wrap">
                        <p class="pricing">
                            @foreach ($getitem->variation as $key => $value)
                                <h3 id="temp-pricing" class="product-price">{{$getdata->currency}}{{number_format($value->product_price, 3)}}</h3>
                                @if ($value->sale_price > 0)
                                    <h3 id="card2-oldprice">{{$getdata->currency}}{{number_format($value->sale_price, 3)}}</h3>
                                @endif
                                @break
                            @endforeach
                        ...
                        <div class="pro-box">
                            ...
                            <div class="product-details-wrap">
                                <div class="product-details">
                                    ...
                                    <p class="pro-pricing">
                                        @foreach ($item->variation as $key => $value)
                                            {{$getdata->currency}}{{number_format($value->product_price, 3)}}
                                            @break
                                        @endforeach
                                    </p>
                                </div>
                                <div class="product-details">
                                    <p>{{ Str::limit($item->item_description, 100) }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
...
$('input[type="checkbox"]').change(function() {
    ...
    $('p.pricing').text('{{$getdata->currency}}'+total.toFixed(3));
    $('#price').val(total.toFixed(3));
...
$(".readers").change(function() {
    ...
    $(".readers option:selected").each(function() {
        ...
        $('p.pricing').text('{{$getdata->currency}}'+ttl.toFixed(0));
        if (ttlsaleprice > 0) {
            $('p.card2-oldprice-show').text('{{$getdata->currency}}'+ttlsaleprice.toFixed(0));
        }
